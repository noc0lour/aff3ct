...
: Decoder_SIHO<B, R> (K, is_closed ? 2 * K + 2 * static_cast<int>(std::log2(trellis[0].size())) : 2 * K),
  ...
  m_bin_vals         ( std::vector<std::array<double, 2>> {{0.0, 0.0}, {0.0, 1.0},
                                                           {1.0, 1.0}, {1.0, 0.0}}                     ),
...
_forward_pass(const R* Y_N)
{
	...
	{
		// LLR extraction
		std::array<R, 2> channel_input {Y_N[2 * (i_step - 1)],
                                             Y_N[2 * (i_step - 1) + 1]};
		...
		for (auto i_state = 0 ; i_state < m_n_states ; i_state++)
		{
			for (auto bit_sys = 0 ; bit_sys < 2 ; bit_sys++)
			{
				...
				if (m_P[previous_node_idx] != DOUBLE_INF)
				{
					...
					const double new_weight = node_weight 
					                          + channel_input[0] * output[0] 
					                          + channel_input[1] * output[1];
	...
	{
		std::array<R, 2> channel_input {Y_N[2 * i_step],
                                             Y_N[2 * i_step + 1]};
