...
template<typename B, typename Q>
Decoder_Viterbi_list_parallel<B,Q>* Decoder_Viterbi_list_parallel<B,Q>
::clone() const
{
...
::__forward_pass(const Q* Y_N)
{
    ...
    for (int i_step = 1 ; i_step <= m_K ; i_step++)
    {
        ...
        std::array<Q, 2> channel_input {Y_N[2 * (i_step - 1)],
                                             Y_N[2 * (i_step - 1) + 1]};
...
template<typename B, typename Q>
void Decoder_Viterbi_list_parallel<B,Q>
::_process_step(const int i_step, const int i_next_state)
{
    ...
    {
        const int idx = l * (m_n_steps + 1) * m_n_states + i_step * m_n_states + i_next_state; // rang ii, etape i_step, etat i_state
...
template<typename B, typename Q>
void Decoder_Viterbi_list_parallel<B,Q>
::__forward_pass_closing(const Q* Y_N)
{
    // = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = FERMETURE
                    ...
                    for (auto i_rank = 0 ; i_rank < m_L ; i_rank++)
                    {
                        ...
                        if (m_P[prev_node_idx] != gDOUBLE_INF)
                        {
                            ...
                            if (branch_weight == gDOUBLE_INF) // pas calcule
                            {
                                ...
                                m_branch_metr[i_next_state * (m_K + m_n_memories) + previous_state] = branch_weight; // stockage
                            }
...
template<typename B, typename Q>
void Decoder_Viterbi_list_parallel<B,Q>
::__backwards_pass(const int l, std::vector<B>& data) /* write in data, name and type not defined yet */
{
    ...
    for (auto i_step = m_K + m_n_memories; i_step > 0 ; i_step--)
    {
        ...
        if (i_step - 1 < m_K)
            data[i_step - 1] = m_M[previous_state * m_n_states + state] - 1;
...
template<typename B, typename Q>
void Decoder_Viterbi_list_parallel<B,Q>
::_backwards_pass(std::vector<B>& X_N)
{
    std::vector<B> decoded_path (m_K);
    // compute first path separately, so that we may still return something in the end
    __backwards_pass(0, decoded_path);
...
template<typename B, typename Q>
size_t Decoder_Viterbi_list_parallel<B,Q>
::_argmin(Q* ptr, size_t size)
{
